...
async def user_revoked(user: UserCRUDRequest, request: Request) -> OperationResultResponse:
    ...
    try:
        ...
        dbuser = await User.create_or_update(callsign=user.callsign, rmuuid=user.uuid, cert_pem=user.x509cert)
...
async def user_promoted(user: UserCRUDRequest, request: Request) -> OperationResultResponse:
    ...
    require_rm_caller(request)
    await User.create_or_update(callsign=user.callsign, rmuuid=user.uuid, cert_pem=user.x509cert, is_rmadmin=True)
    return OperationResultResponse(success=True)
...
async def user_demoted(user: UserCRUDRequest, request: Request) -> OperationResultResponse:
    ...
    require_rm_caller(request)
    await User.create_or_update(callsign=user.callsign, rmuuid=user.uuid, cert_pem=user.x509cert, is_rmadmin=False)
    return OperationResultResponse(success=True)
